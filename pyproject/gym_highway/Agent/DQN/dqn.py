...
class StateProcessor():
    ...
    def __init__(self):
        ...
        with tf.variable_scope("state_processor"):
            self.input_state = tf.placeholder(shape=[17, ], dtype=tf.float32)
...
def deep_q_learning(sess,
    ...
    for i in range(replay_memory_init_size):
        ...
        # sess.run(q_estimator.train_op, {q_estimator.X_pl: np.reshape(state, [1, 17]),
        #                                          q_estimator.y_pl: np.reshape(action, [1, 1]),
        #                                          q_estimator.actions_pl: np.reshape()})
        # next_state = np.append(state, np.expand_dims(next_state, 1), axis=0)
    ...
    for i_episode in range(num_episodes):
        ...
        for t in itertools.count():
            ...
            action = np.random.choice(np.arange(len(action_probs)), p=action_probs)
            next_state, reward, done, _ = env.step(VALID_ACTIONS[action])
            # next_state = sess.run(q_estimator.updates, {q_estimator.X_pl: np.reshape(state, [1, 17]),
            #                                            q_estimator.y_pl: np.reshape(action, [1, 1])})
