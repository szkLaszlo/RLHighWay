...
from gym_highway.lib import plotting
...
def play_episodes(environment, n_episodes, polic):
    wins = 0
    total_reward = 0
    ...
    for episode in range(n_episodes):
        ...
        while not terminated:
            ...
            if terminated and info == 'Everything is fine':
                wins += 1
...
def q_learning(env, estimator, num_episodes, discount_factor=1.0, epsilon=0.1, epsilon_decay=1.0):
    ...
    policy = make_epsilon_greedy_policy(
        estimator, epsilon * epsilon_decay ** 1, nA)
...
stats, policy = q_learning(env, estimator, 100, epsilon=0.4)
print(np.sum(stats.episode_lengths) / stats.episode_lengths.__len__())
print(np.sum(stats.episode_rewards) / stats.episode_rewards.__len__())
# In[124]:
